...
/* Iterative method to simplify the tree, each call to the root of the tree descends until
 * it finds a node with children who are both numbers, and then simplifies, e.g.
 *
 * <tree>
 *     +
 *    / \
 ...
 * </tree>
 *
 * this is called repeatedly until the tree's root is not an operator
 */
...
//		display_list_root_values(li_gen);
		...
		// get right child
		t_left = c2->get_next()->data;
		// pop
		li_gen->remove_node(c2->get_next());
		// get left child
		t_right = c2->get_next()->data;
		// pop
		li_gen->remove_node(c2->get_next());
